    ...
    try {
        ...
        if (!imageBase64) {
            const chatCompletion = await groq.chat.completions.create({
                messages: [
                    { role: "system", content: "أنت مساعد خبير في كتابة إعلانات المنتجات المستعملة بالعربية." },
                    { role: "user", content: `اكتب وصفاً جذاباً لمنتج: ${title}، فئة: ${category}، حالة: ${condition}.` }
                ],
        ...
        res.status(500).json({ success: false, message: "حدث خطأ في الـ AI" });
    }
};
