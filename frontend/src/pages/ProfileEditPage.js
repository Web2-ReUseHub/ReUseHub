...
export default function ProfileEditPage() {
  ...
  const [avatar, setAvatar] = useState(null);         // الملف الفعلي
  const [avatarPreview, setAvatarPreview] = useState(null); // للعرض
  ...
  const handleSave = async () => {
    try {
      ...
      // ✅ رفع الصورة كملف إذا اختار المستخدم صورة جديدة
      if (avatar && avatar instanceof File) {
        ...
        avatar_url = uploadRes.data.avatar_url; // السيرفر يرجع المسار
      }

      const body = { f_name, l_name, phone, city, bio, username };
      if (avatar_url) body.avatar_url = avatar_url; // بس حدّث إذا في صورة جديدة

      await axios.put(`${API}/user/profile/update`, body, {
        headers: { Authorization: `Bearer ${token}` },
      });

      setSaved(true);
      setTimeout(() => { setSaved(false); navigate("/profile"); }, 2000);
    } catch (error) {
      ...
      <div style={{ maxWidth: 780, margin: "40px auto", padding: "0 20px 60px" }}>
        ...
        <div style={{ background: CARD_BG, borderRadius: 16, border: `1px solid ${BORDER}`, padding: "28px 32px", marginBottom: 20, boxShadow: "0 2px 12px rgba(27,43,94,0.05)" }}>
          ...
          <div style={{ marginTop: 18 }}>
            ...
            <textarea
              ...
              style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: `1.5px solid ${BORDER}`, fontSize: 14, color: TEXT_MAIN, fontFamily: "inherit", resize: "vertical", outline: "none", background: LIGHT_BG, boxSizing: "border-box", direction: "rtl" }}
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "flex-start" }}>
          <button
            onClick={handleSave}
            disabled={loading}
            style={{ background: NAVY, color: "#fff", border: "none", borderRadius: 10, padding: "12px 32px", fontWeight: 700, fontSize: 15, cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1 }}
          >
        ...
        {saved && (
          <div style={{ position: "fixed", bottom: 30, left: "50%", transform: "translateX(-50%)", background: NAVY, color: "#fff", padding: "14px 28px", borderRadius: 12, fontWeight: 600, fontSize: 15, zIndex: 999 }}>
  ...
  return (
    <div>
      ...
      <input
        ...
        dir={dir || "rtl"}
        style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: `1.5px solid ${focused ? "#1B2B5E" : "#E0E4EF"}`, fontSize: 14, color: "#1B2B5E", fontFamily: "inherit", outline: "none", background: "#F5F6FA", boxSizing: "border-box" }}
