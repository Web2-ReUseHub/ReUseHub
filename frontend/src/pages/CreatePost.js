...
import { authFetch } from "../components/api";
...
const [aiError, setAiError] = useState('');
 ...
 const analyzeWithAI = async () => {
  if (!images.length) {
    setAiError("ارفع صورة أولاً");
    return;
  }

  if (!price) {
    setAiError("أدخل السعر أولاً");
    return;
  }
  ...
  try {
    ...
    const res = await fetch("http://localhost:5004/api/ai/generate-post", {
      method: "POST",
      headers: {
    ...
    <div style={page}>
      <div style={card}>
        ...
        <div style={headerBlock}>
          ...
          <p style={subtitle}>
            رتّب بيانات المنتج بشكل واضح، وأضف صوراً جذابة لتزيد فرصة البيع بشكل أسرع.
          </p>
        </div>
        ...
        <div style={formGrid}>
          ...
          <div style={uploadSection}>
            ...
            <div onClick={openFilePicker} style={uploadBox} role="button" tabIndex={0}>
              ...
              <button type="button" style={uploadAction}>
                اختيار الصور
              </button>
            </div>
     ...
     {aiError && (
          ...
          <button type="button" onClick={analyzeWithAI} disabled={aiLoading}
            style={{ ...submitBtn, background: aiLoading ? "#8ca0c0" : "#0f7b5e", marginBottom: "4px" }}>
            {aiLoading ? "⏳ جاري التحليل..." : "✨ تحليل الصورة بالذكاء الاصطناعي"}
          </button>
...
const card = {
  ...
  boxShadow: "0 20px 55px rgba(19, 42, 90, 0.10)"
};
...
const activeChip = {
  ...
  background: "#1a3970",
  color: "#ffffff",
  boxShadow: "0 10px 20px rgba(26, 57, 112, 0.18)"
};
...
const submitBtn = {
  ...
  borderRadius: "18px",
  border: "none",
  background: "linear-gradient(135deg, #16366b 0%, #244c8f 100%)",
  color: "#ffffff",
  fontSize: "18px",
  fontWeight: 800,
  cursor: "pointer",
  boxShadow: "0 18px 35px rgba(22, 54, 107, 0.22)"
};
